  ...
  return <PageFrame>
    <main className="product-page">
      ...
      <div className="product-detail">
        <section className="product-gallery"><div className="product-detail-image"><Image src={product.image} alt={`${product.name}, pieza ${product.piece} de OVRLMT`} fill priority sizes="(max-width: 900px) 100vw, 58vw" /></div><div className="product-gallery-rail"><div><Image src={product.image} alt={`Detalle frontal de ${product.name}`} fill sizes="240px" /></div><div><Image src={product.image} alt={`Detalle de impresión de ${product.name}`} fill sizes="240px" /></div></div></section>
        <section className="product-detail-info">
          <p className="product-status"><i /> SOBRE PEDIDO / {getFomoLabel(stockSummary.available)}</p>
          ...
          <div className="technical-details"><p className="section-label">DETALLES TÉCNICOS</p><dl className="product-specs"><div><dt>COLOR</dt><dd>{product.color}</dd></div><div><dt>FIT</dt><dd>{product.fit}</dd></div><div><dt>MATERIAL</dt><dd>{product.material || PRODUCT_MATERIAL}</dd></div><div><dt>IMPRESIÓN</dt><dd>{product.printMethod}</dd></div><div><dt>EDICIÓN</dt><dd>{product.units} cupos por diseño</dd></div><div><dt>DISPONIBILIDAD</dt><dd>{stockSummary.available} cupos</dd></div><div className="wide"><dt>PRODUCCIÓN</dt><dd>Sobre pedido, después de confirmar el pago</dd></div><div className="wide"><dt>ENTREGA LOCAL</dt><dd>{LOCAL_DELIVERY_COPY}</dd></div><div className="wide"><dt>USO RECOMENDADO</dt><dd>Outfit urbano, nocturno, casual y motorsport inspired</dd></div><div className="wide"><dt>CUIDADOS</dt><dd>Lavar al revés, agua fría, no usar cloro, no planchar sobre el estampado</dd></div></dl></div>
