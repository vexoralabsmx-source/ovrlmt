...
import { SIZES, WHATSAPP_NUMBER } from "@/data/store";
...
export function PreorderForm() {
  ...
  function submit(e: FormEvent<HTMLFormElement>) {
    ...
    setSent(true); window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`, "_blank", "noopener,noreferrer");
  }
  return <form className="preorder-form" onSubmit={submit}>
    ...
    <label><span>05 / TALLA</span><select name="size" defaultValue="M">{SIZES.map(s => <option key={s}>{s}</option>)}</select></label>
